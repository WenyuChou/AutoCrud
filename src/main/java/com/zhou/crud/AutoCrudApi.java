...
public class AutoCrudApi {
    /**<font color = yellow>请使用下述main方法生成代码。</font>*/
    ...
    public void createJavaCode(String database, String tableName) {
        if (createPojo(this.parentPage, database, tableName)) {
            log.info(pojo + " 创建成功");
        }
        if (createDao(this.parentPage, database, tableName)) {
            log.info(dao + " 创建成功");
        }
    }
    /*-------------------------------------------数据库操作-----------------------------------------------------------*/
    ...
    public boolean createDao(String parentPage, String database, String tableName) {
        ...
        File dao = new File(file.getAbsoluteFile() + "/" + nameChange(tableName, true) + "Mapper.java");
        if (!dao.exists()) {
            String importJar = "import " + parentPage + "." + this.pojo + "." + nameChange(tableName, true) + ";" +
            ...
            try {
                bw = new BufferedWriter(new FileWriter(file.getAbsoluteFile() + "/" + nameChange(
                        tableName, true) + "Mapper.java"));
                //在创建好的文件中写入"具体代码"
                bw.write("package " + parentPage + "." + packageName + ";\n" + importJar + "\n" + getAuthor() + "\n" +
                        "@Mapper\npublic interface " + nameChange(tableName, true) + "Mapper{\n" +
                        "    int " + this.sqlInsert + "(" + nameChange(tableName, true) + " param);\n" +
                        "    int " + this.sqlDelete + "(Long " + nameChange(columnNames.get(0), false) + ");\n" +
                        "    int " + this.sqlUpdate + "(" + nameChange(tableName, true) + " param);\n" +
                        "    List<" + nameChange(tableName, true) + "> " + this.sqlSelectList + "(" + nameChange(tableName, true) + " param);\n" +
                        "    List<" + nameChange(tableName, true) + "> " + this.sqlSelectListByIds + "(@Param(\"ids\")Set<Long> ids);\n" +
                        "    " + nameChange(tableName, true) + " " + this.sqlSelectObject + "(" + nameChange(tableName, true) + " param);\n" +
                        "    " + nameChange(tableName, true) + " " + this.sqlSelectById + "(Long " + nameChange(columnNames.get(0), false) + ");\n" +
                        "\n}");
                //一定要关闭文件
                bw.close();
            } catch (IOException e) {
            ...
            log.info(nameChange(tableName, true) + "Mapper.java 已经存在(未覆盖，如需重建则先删除此文件)");
        }
        ...
        if (new File(mapperPage.getAbsolutePath() + "/" + nameChange(tableName, true) + "Mapper.xml").exists()) {
            log.info(mapperPage.getAbsoluteFile() + "/"
                    + nameChange(tableName, true) + "Mapper.xml 已经存在(未覆盖，如需重建则先删除此文件)");
            return false;
        }
        String pojoPath = "\"" + parentPage + "." + this.pojo + "." + nameChange(tableName, true) + "\"";
        StringBuilder code = new StringBuilder
                ("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                        "<!DOCTYPE mapper PUBLIC \"-//mybatis.org//DTD Mapper 3.0//EN\" \"http://mybatis.org/dtd/mybatis-3-mapper.dtd\">"
                        + "\n<mapper namespace=\"");
        code.append(parentPage).append(".").append(this.dao).append(".").append(nameChange(tableName, true)).append("Mapper\">\n")
        ...
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(mapperPage.getAbsolutePath() + "/"
                    + nameChange(tableName, true) + "Mapper.xml"));
